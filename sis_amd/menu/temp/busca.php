<?php
	...
	$resultado = file_get_contents("https://api.thingspeak.com/channels/406317/feeds.json?api_key=ZBD3P1L1TPWIQOOE");
	...
	function gen_data1($dados, $eixo){
		...
		if($eixo == 0){
			for($i = 0; $i < count($var); $i++){
				$data .= '"label": "' . $var[$i]->{'field1'} . '", "value": "' . $var[$i]->{'field3'} . '"';
				$data .= $i == (count($var) - 1) ? '' : '},{';
			}
		}
		else if($eixo == 1){
			for($i = 0; $i < count($var); $i++){
				$data .= '"label": "' . $var[$i]->{'field2'} . '", "value": "' . $var[$i]->{'field3'} . '"';
				$data .= $i == (count($var) - 1) ? '' : '},{';
			}
		}
	...
	function listar_horas($date){
		...
		for($i = 0; $i < count($var); $i++) {
			if($var[$i]->{'field1'} == $date) {
				$x = substr($var[$i]->{'field2'}, 0, 2);
	...
	function medias_h($date, $horas, $v1 = 0, $v2 = 9999999999) {
		global $resultado;
	...
	function filtro_medias($valores, $medias, $qtd, $pag){
		...
		for($j = 0; $j < count($medias); $j++){
			if($c1 <= $j && $c2 > $j){
				...
				else if(strlen($valores[$j]) == 2) $v = $valores[$j] . ':00:00';
				else $v = $valores[$j];
