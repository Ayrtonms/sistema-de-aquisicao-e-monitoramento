...
public class BuscaL {
	...
	public JSONArray getDados1(int d1, int d2, int h1, int h2, double v1, double v2, int qtd, int pag) {
	...
	public ArrayList<String> getHoras(String date) {
		ArrayList<String> horas = new ArrayList<String>();

		for (int i = 0; i < feeds.length(); i++) {
			JSONObject obj = feeds.getJSONObject(i);

			if (date.equals(obj.getString("field1"))) {
				String hora = obj.getString("field2").substring(0, 2);
	...
	public JSONArray getDados2(ArrayList<String> dias, int h1, int h2, double v1, double v2) {
		JSONArray result = new JSONArray();
		...
		for (int i = 0; i < dias.size(); i++) {
			...
			novo.put("valor", c == 0 ? 0 : Math.round(total * 100 / c) / 100.0);
	...
	public JSONArray getDados3(ArrayList<String> meses, int h1, int h2, double v1, double v2) {
		JSONArray result = new JSONArray();
	...
	public JSONArray getDados4(String date, ArrayList<String> horas, double v1, double v2) {
		JSONArray result = new JSONArray();

		for (int i = 0; i < horas.size(); i++) {
			String hora = horas.get(i);
			String form = hora + ":00:00";
			...
			for (int j = 0; j < feeds.length(); j++) {
				...
				if (date.equals(obj.getString("field1"))) {
					String y = obj.getString("field2").substring(0, 2);
					double z = obj.getDouble("field4");

					if (hora.equals(y) && v1 <= z && v2 >= z) {
						total += z;
						c++;
					}
				}
	...
	public static JSONArray formatDados(JSONArray list, int qtd, int pag) {
		...
		for (int i = 0; i < list.length(); i++) {
			if (c1 <= i && c2 > i) {
				...
				novo.put("data", data);
				novo.put("valor", obj.getDouble("valor"));
	...
	public static JSONArray formatDados2(JSONArray list, int qtd, int pag) {
		JSONArray result = new JSONArray();
